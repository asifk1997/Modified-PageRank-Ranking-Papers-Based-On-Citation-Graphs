...
all_elements = {"article", "inproceedings", "proceedings", "book", "incollection", "phdthesis", "mastersthesis", "www","person","data"}
...
def parse_inproceedings(dblp_path, save_path, save_to_csv=False, include_key=False):
    ...
    type_name = all_elements
    features = ['title', 'author', 'year', 'journal', 'booktitle','cite']
...
def main(name):
    dblp_path = file_path
    save_path = 'dataset/proceedings.json'
    try:
        ...
        exit()
    parse_inproceedings(dblp_path=dblp_path,save_path=save_path,include_key=True,save_to_csv=False)
